...
function mergeRelationshipMemory(
  ...
  if (!parsed || targetTurnCount <= currentMemory.lastDiaryTurnCount) {
  ...
  const nextMemory: RelationshipMemory = {
    ...currentMemory,
    version: 2,
    lastDiaryTurnCount: Math.min(targetTurnCount, currentMemory.turnCount),
    mood: normalizeRelationshipMood(parsed['mood']),
    trust: clampRelationshipStat(currentMemory.trust + getDelta(parsed['trustDelta'])),
    attraction: clampRelationshipStat(currentMemory.attraction + getDelta(parsed['attractionDelta'])),
    respect: clampRelationshipStat(currentMemory.respect + getDelta(parsed['respectDelta'])),
    irritation: clampRelationshipStat(currentMemory.irritation + getDelta(parsed['irritationDelta'])),
