...
export function initPostProcessing(
  ...
  const opacityUniform = bleachBypassPass.uniforms['opacity'];
  if (opacityUniform) {
    opacityUniform.value = 0.2;
  }
  ...
  const fxaaResolutionUniform = fxaaPass.material.uniforms['resolution'];
  if (fxaaResolutionUniform) {
    fxaaResolutionUniform.value.x = 1 / (window.innerWidth * pixelRatio);
    fxaaResolutionUniform.value.y = 1 / (window.innerHeight * pixelRatio);
  }
...
export function resizePostProcessing(refs: PostProcessingRefs, renderer: THREE.WebGLRenderer) {
